					...
					<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<td class="label">
							<label for="product-<?php echo esc_attr($grouped_product->get_id()); ?>">
								<?php print $grouped_product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name(); ?>
							</label>
						...
						<td class="price">
							<?php
								print $grouped_product->get_price_html();
